...
const ChessGunPlayer1 = ({ gun, address, player, gameId, writeContracts, startFen, lastGunState, tx }) => {
  ...
  const [gunState, setGunState] = useState(lastGunState);
  const [nonce, setNonce] = useState(0);
  const [turn, setTurn] = useState(lastGunState.turn);
          ...
          <Modal footer="" visible={winningModalVisible} onOk={executeWin}>
            <Space>
              <Card>
                ...
                <p>If no dispute is entered within 7 blocks, execute the final transaction and gather the winnings!</p>
